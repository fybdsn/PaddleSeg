...
from paddle.nn import AdaptiveAvgPool2D, BatchNorm, Conv2D, Dropout, Linear, Conv3D, AdaptiveAvgPool3D
from medicalseg.cvlibs import manager
from medicalseg.utils import utils, logger
...
class ConvBNLayer(nn.Layer):
    def __init__(self,
        ...
        self.conv = Conv3D(
            ...
            bias_attr = False,
            dilation = dilation)
        self.bn = nn.BatchNorm3D(num_features = out_c,
...
@manager.BACKBONES.add_component
def MobileNetV3_large_x1_0_nnunet(**kwargs):
    cfg = {
        "model": [
        # k, exp, c, se, act, s, {d}
        [3, 16, 16, False, "relu", 1],
        ...
        "out_index":[0,2, 5, 11, 14],
        "stages_pattern_large":
            ["blocks[0]", "blocks[2]", "blocks[5]", "blocks[11]", "blocks[14]"]
    }
    out_ch = [16,24,40,112,160]
    model = MobileNetV3(
        config=cfg['model'],
        scale=1.0,
        stages_pattern=cfg['stages_pattern_large'],
...
#     model,_ = MobileNetV3_large_x1_0_nnunet(in_channels=1)
#     model.eval()
#     x = paddle.rand((1,1,32,32,32))
#     o = model(x)
#     print('out:')
#     for i in o:
#         print(i.shape)
